...
> **Важно по Next.js:** Значения `NEXT_PUBLIC_` переменных "запекаются" в бандл на этапе сборки (`next build`). Изменение флага в рантайме в production не сработает — потребуется пересборка.
